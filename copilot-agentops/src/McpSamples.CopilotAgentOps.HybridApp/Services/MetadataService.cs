...
using McpSamples.CopilotAgentOps.HybridApp.Models;

namespace McpSamples.CopilotAgentOps.HybridApp.Services;

/// <summary>
/// This represents the service entity for searching and loading custom instructions from the copilot-agentops repository.
/// </summary>
public class MetadataService(HttpClient http, JsonSerializerOptions options, ILogger<MetadataService> logger) : IMetadataService
{
    ...
    private const string CopilotAgentOpsFileUrl = "https://raw.githubusercontent.com/ajilantony/copilot-agentops/refs/heads/main/{directory}/{filename}";
    ...
    public async Task<string> LoadAsync(string directory, string filename, CancellationToken cancellationToken = default)
    {
        ...
        var url = CopilotAgentOpsFileUrl.Replace("{directory}", directory).Replace("{filename}", filename);
        try
        {
